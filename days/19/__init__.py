...
class Factory:
    ...
    def find_best_path(self, robots, resources, i):
        ...
        for possible in possibles:
            ...
            if (
                ...
                or (
                    possible[0][2] == 0
                    and self.earliest_obsidian_bot_turn
                    and i + 1 > self.earliest_obsidian_bot_turn
                )
                or (
                    possible[0][2] == 0
    ...
    @functools.cache
    def find_best_path_2(self, robots, resources, i):
        ...
        # if self.go_in_geode_robot_mode_turn is None and go_in_geode_robot_mode) or (
        #     self.go_in_geode_robot_mode_turn and i < self.go_in_geode_robot_mode_turn
        # ):
        if go_in_geode_robot_mode:
            self.go_in_geode_robot_mode_turn = i
            self.num_geodes_when_going_mode = resources[3]
            x = self.hope(i, resources[3], robots[3])
            return x
        ...
        for possible in possibles:
            ...
            if (
                ...
                or (
                    self.go_in_geode_robot_mode_turn is not None
                ...
                continue
            num_geodes = self.find_best_path_2(*possible)
            if num_geodes > self.current_best:
                self.current_best = num_geodes
        return self.current_best
...
def do_part_2(blueprints):
    ...
    for name, blueprint in blueprints[:3]:
        ...
        m = factory.find_best_path_2((1, 0, 0, 0), (0, 0, 0, 0), 0)
        print(f"{factory.name} - max: {m}")
        geodes.append((m, int(factory.name)))
        maxes.append(m)
    print(geodes)
